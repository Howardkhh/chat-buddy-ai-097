...
export default function VoiceCaptionOverlay({
  open,
  captions,
  onClear,
  topOffset = 0,
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b">
          <div className="flex items-center gap-3">
            ...
            {/* dB label */}
            <span className="text-xs text-muted-foreground tabular-nums">{Math.round(db)} dB</span>

            {/* threshold label */}
            <span className="text-xs text-red-600 tabular-nums">(thr {Math.round(thresholdDb)} dB)</span>
          </div>
        ...
        <ScrollArea className="h-[calc(100%-48px)] px-4 py-3">
          {captions.length === 0 ? (
            <div className="text-center text-muted-foreground py-10">
              The conversation captions will appear here…
            </div>
          ) : (
